...
sync_karing_system_helper() {
  ...
  local sudoers_rule_dst="/etc/sudoers.d/49-karing-tun"
...
EOF
    ...
    cat > /tmp/49-karing-tun.sudoers <<EOF
$desired_user ALL=(root) NOPASSWD: /usr/bin/chown
$desired_user ALL=(root) NOPASSWD: /usr/bin/chmod +sx /nix/store/*/share/karing/karingService
EOF
    ...
    local sudoers_needs_sync=1
    if [ -f "$sudoers_rule_dst" ] && cmp -s /tmp/49-karing-tun.sudoers "$sudoers_rule_dst"; then
      sudoers_needs_sync=0
    fi

    if [ "$helper_needs_sync" -eq 1 ] || [ "$rule_needs_sync" -eq 1 ] || [ "$sudoers_needs_sync" -eq 1 ]; then
      echo "[hms] syncing karing system helper, polkit rule, and sudoers rule"
      if ! env SHELL=/bin/sh pkexec "$runtime_shell" -c '
        ...
        install -m 0440 /tmp/49-karing-tun.sudoers "'$sudoers_rule_dst'"
        chown root:root "'$sudoers_rule_dst'"
      '; then
    ...
    rm -f /tmp/49-karing-tun.sudoers
  fi
}
